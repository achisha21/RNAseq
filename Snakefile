...
#trim -> fastqc -> mapping -> sort bam -> index bam -> featurecounts
...
rule index_bam:
    ...
    resources: time_min=360, mem_mb=2000, cpus=1
    output:
        index_bam_file = config["dir_names"]["sorted_dir"] + "/{sample_id}.bam.bai"
    shell: "samtools index {input.index_bam} {output.index_bam_file}"

rule featurecount:
    input:
        expand("{dir}/{sample_id}.sorted.bam",dir=config["dir_names"]["sorted_dir"], sample_id=sample_ids),
        expand("{dir}/{sample_id}.bam.bai",dir=config["dir_names"]["sorted_dir"], sample_id=sample_ids)
    resources: time_min=360, mem_mb=20000, cpus=6
    output:
